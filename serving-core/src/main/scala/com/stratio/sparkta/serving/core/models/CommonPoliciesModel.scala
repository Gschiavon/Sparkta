...
object CommonPoliciesValidator extends SparktaSerializer {
  ...
  def validateDto(aggregationPoliciesDto: CommonPoliciesModel): (Boolean, String) = {
    val (isValidAgainstSchema: Boolean, isValidAgainstSchemaMsg: String) = validateAgainstSchema(aggregationPoliciesDto)

    val isValidDurationGranularity = aggregationPoliciesDto
      .cubes
      .forall(cube =>
        OperationsHelper.checkValidCheckpointInterval(
          aggregationPoliciesDto.sparkStreamingWindow,cube.checkpointConfig.checkpointEvery.get))
  ...
  def validateAgainstSchema(aggregationPoliciesDto: CommonPoliciesModel): (Boolean, String) = {
